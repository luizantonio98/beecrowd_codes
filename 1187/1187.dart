...
bool acimaDiagonalPrincipal(i, j)
{
  if (j > i)
  {
    //print("[$i] e [$j] está acima da diagonal principal - Check");
    return true;
  }
  else
  {
    //print("[$i] e [$j] está abaixo da diagonal principal - Anulado");
    return false;
  }
}
...
bool acimaDiagonalSecundaria(i, j, ordem)
{
  ...
  if (j > i && (i+j < ordem))
  {
    //print("[$i] e [$j] está acima da diagonal secundária - Check");
    return true;
  }
    
  else
  {
    //print("[$i] e [$j] está abaixo da diagonal secundária - Anulado");
    return false;
  }
}
      ...
      for(int j = 0; j < ordem; j++) 
      {
        if (diagonalPrincipal(i,j)== false)
          if (diagonalSecundaria(i,j, ordem) == false)
            if (acimaDiagonalSecundaria(i, j, ordem))
              if (acimaDiagonalPrincipal(i, j))
              {
                soma += matriz[i][j];
                contador ++;
              }
